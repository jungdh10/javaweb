...
<body>
	...
	닉네임
	<input type="text" id="nick" size="30" />
	<br /> 보낼내용
	<input type="text" id="message" size="50" />
	<br />
	<input type="button" id="send" value="보내기" />br /> 받은내용
	<textarea id="print" cols="80" rows="15"></textarea>
	<br />
</body>
<!-- jquery 사용을 위한 링크 설정 -->
<script
...
<script>
	var websocket = new WebSocket("ws://192.168.10.124:8080/0911etc/websocket")
	//에러가 발생한 경우
		...
		$.ajax({
			...
			success : function(data) {
				...
				$(data).find('data').each(function(index, item) {
					...
					output += '<p>' + $(this).find('tmEf').text() + '</p>'
					output += '<p>' + $(this).find('wf').text() + '</p>'
					if (index % 6 == 0) {
						output += '</div>'
					}
